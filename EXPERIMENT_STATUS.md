...
| A-retrain | posterior head, bin-wise warm start, retrained (issue A) | DONE (head-only variant running) | `outputs/posterior/posterior_{r2,r8}_warm/`, `results/E21_posterior_rays/*_warm_test.json`, `results/E30_stage_d/*_warm_v3*/` | head-only variant | best epoch 4 / 2; every Stage D number within 0.01 of the flat head; C still 0.04–0.05 below A at N=all under both references → limitation confirmed | close after head-only | see git log |
